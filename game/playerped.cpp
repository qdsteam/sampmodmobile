...
void CPlayerPed::EnterVehicle(int iVehicleID, bool bPassenger)
{
	...
	if(bPassenger && pVehicle->m_bDoorsLocked) {
    		TogglePlayerControllable(false);
			Timer timer;
			timer.add(std::chrono::milliseconds(3000), unlockPedTask, true);
			return;
	}
	...
	if(bPassenger && !pVehicle->m_bDoorsLocked)
	{
		...
		{
			if(!IN_VEHICLE(m_pPed) && !pVehicle->m_bDoorsLocked)
